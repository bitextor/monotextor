...
def parse_args():
    oparser = argparse.ArgumentParser(
        description="launch Monotextor", add_help=False)
    config = oparser.add_argument_group("Monotextor config")

    config.add_argument("-C", "--configfile", dest="configfiles", metavar="FILE",
                        help="Monotextor YAML configuration file", nargs='+')
    config.add_argument("-c", "--config", dest="config", metavar="KEY=VALUE", nargs="+", action=ParseDict,
                        help="Set or overwrite values for Monotextor config")
    ...
    if not options.config and not options.configfiles:
        oparser.error("provide Monotextor configuration via --configfile or --config")

    return options


# parsing Monotextor config passed a list of KEY=VALUE pairs
